...
if (isset($_GET['WHERE'])) {
		...
		//$query = "SELECT count(*) AS count , tag4f AS tag4f FROM tracciato  WHERE ".urldecode($_GET['WHERE'])."   GROUP BY tag4f ORDER BY tag4f ASC";	
		 $query = "SELECT count(*) AS count, SUBSTRING_INDEX(tag4f,' ',1) AS tag4f FROM tracciato WHERE ".urldecode($_GET['WHERE']).  " GROUP BY SUBSTRING_INDEX(tag4f,' ',1) ORDER BY tag4f  ASC";
	}else {
		//$query = "SELECT count(*) AS count, tag4f AS tag4f FROM tracciato  GROUP BY tag4f ORDER BY tag4f ASC";
		  $query = "SELECT count(*) AS count, SUBSTRING_INDEX(tag4f,' ',1) AS tag4f FROM tracciato  GROUP BY SUBSTRING_INDEX(tag4f,' ',1) ORDER BY tag4f  ASC";
	}
