...
function dashboard() {
    return {
        ...
        async init() {
            ...
            if (token && userStr) {
                try {
                    ...
                    console.log('Found saved session for:', this.user.email);

                    // IMPORTANTE: Valida la sessione PRIMA di mostrare la dashboard
                    ...
                    this.$nextTick(() => {
                        setTimeout(() => {
                            ...
                            if (this.user?.isAdmin) this.loadSystemInfo(); // Carica info sistema (admin)
                            this.loadStores();
                            this.loadAllData();
                        }, 300);
                    });
        ...
        async login() {
            ...
            try {
                ...
                if (data.forcePasswordChange) {
                    ...
                    // Set flag to prevent auto-reload during password change
         ...
         * @returns {boolean} true se il refresh è riuscito
         */
        async tryRefreshToken() {
            ...
            try {
                const response = await fetch('/merchant-api/api/v2/auth/refresh', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
        ...
        /**
         * Fetch wrapper che gestisce automaticamente il refresh del token su 403
        ...
        /**
         * Verifica se la sessione è ancora valida all'avvio
        ...
        transactionsRequestCounter: 0, // Counter per evitare race condition

        async loadTransactions() {
            const thisRequestId = ++this.transactionsRequestCounter;
            this.loading = true;
            ...
            try {
                ...
                // Ignora risposta se c'è una richiesta più recente
                if (thisRequestId !== this.transactionsRequestCounter) {
                    console.log('Transactions response #', thisRequestId, 'ignored (newer request exists)');
                    return;
                }
        ...
        chartsRequestCounter: 0, // Counter per evitare race condition

        async loadCharts() {
            const thisRequestId = ++this.chartsRequestCounter;
            ...
            try {
                ...
                // Load circuit distribution and trend in parallel (con refresh automatico)
                const [circuitResponse, trendResponse] = await Promise.all([
                    this.fetchWithAuth(circuitUrl),
                    this.fetchWithAuth(trendUrl)
                ]);

                // Ignora risposta se c'è una richiesta più recente
                if (thisRequestId !== this.chartsRequestCounter) {
                    console.log('Charts response #', thisRequestId, 'ignored (newer request exists)');
                    return;
                }

                console.log('Charts response status:', circuitResponse.status, trendResponse.status);
        ...
        async loadStores() {
            try {
                const response = await this.fetchWithAuth('/merchant-api/api/v2/stores/groups');
        ...
        async exportToCSV() {
            ...
            try {
                ...
                const response = await this.fetchWithAuth(`/merchant-api/api/v2/transactions?${params}`);
        ...
        statsRequestCounter: 0, // Counter per evitare race condition

        async loadStatisticsCharts() {
            const thisRequestId = ++this.statsRequestCounter;
            console.log('Statistics request #', thisRequestId);
            ...
            try {
                ...
                // Ignora risposta se c'è una richiesta più recente
                if (thisRequestId !== this.statsRequestCounter) {
                    console.log('Statistics response #', thisRequestId, 'ignored (newer request exists)');
                    return;
                }
                ...
                console.log('Statistics updated from request #', thisRequestId, '- transactions:', transactions.length);
                if (this.statsFilterStore) {
        ...
        geoRequestCounter: 0, // Counter per evitare race condition

        async loadGeoDistribution(filteredTransactions = null) {
            // Incrementa counter per tracciare questa richiesta
            const thisRequestId = ++this.geoRequestCounter;
            console.log('Geo distribution request #', thisRequestId);
            ...
            try {
                ...
                // Ignora risposta se c'è una richiesta più recente
                if (thisRequestId !== this.geoRequestCounter) {
                    console.log('Geo distribution response #', thisRequestId, 'ignored (newer request exists)');
                    return;
                }

                if (response.ok) {
                    this.geoDistribution = await response.json();
                    console.log('Geo distribution updated from request #', thisRequestId);
        ...
        async loadSystemInfo() {
            ...
            try {
                const response = await this.fetchWithAuth(`${this.apiUrl}/api/v2/admin/system-info`);
                if (response.ok) {
                    this.systemInfo = await response.json();
                    console.log('System info loaded:', this.systemInfo);

                    // Imposta refresh automatico ogni 30 secondi (solo la prima volta)
                    if (!this.systemInfoInterval) {
                        this.systemInfoInterval = setInterval(() => this.loadSystemInfo(), 30000);
                    }
                }
            } catch (e) {
        ...
        async loadUsers() {
            ...
            try {
                ...
                const response = await this.fetchWithAuth(`${this.apiUrl}/api/v2/admin/users?page=0&size=1000${searchParam}`);
                ...
                if (response.ok) {
                    ...
                    // Load stats
                    const statsResponse = await this.fetchWithAuth(`${this.apiUrl}/api/v2/admin/users/stats`);
                    if (statsResponse.ok) {
        ...
        async submitPasswordChange() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    // Remove the flag that prevents auto-reload
        ...
        async loadActivationCodes() {
            ...
            try {
                ...
                const response = await this.fetchWithAuth(`${this.apiUrl}/api/v2/activation-codes?${params}`);
                ...
                if (response.ok) {
                    ...
                    // Load stats
                    const statsResponse = await this.fetchWithAuth(`${this.apiUrl}/api/v2/activation-codes/stats`);
                    if (statsResponse.ok) {
