...
switch ($action) {
            ...
            if (empty($email) || empty($password) || empty($bu)) {
                ...
                if ($checkStmt->get_result()->num_rows > 0) {
                    ...
                    if ($stmt->execute()) {
                        ...
                        $messageType = 'success';
                        $stmt->close();
                        // Redirect per evitare ricaricamento POST
    ...
    case 'update':
        if ($_POST) {
            ...
            $stmt->bind_param("sssiii", $email, $bu, $ragione_sociale, $active, $force_password_change, $id);
...
<div class="container-fluid mt-4">
    ...
    <!-- Pulsante Nuovo Utente e Ricerca -->
    <div class="row mb-3">
        <div class="col-md-6">
        ...
        <div class="col-md-6">
            <div class="input-group">
                <input type="text" class="form-control" id="searchUsers" placeholder="Cerca per email, BU o ragione sociale...">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <?php if ($user['bu'] === '9999'): ?>
                                    <small class="text-warning d-block"><i class="fas fa-crown"></i> Admin</small>
                                <?php endif; ?>
                            </td>
            ...
            <form method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="bu" required placeholder="es. 1234, 9999" maxlength="45">
                        <small class="form-text text-muted">
                            <strong>9999</strong> = Amministratore completo<br>
                            Altri codici = Accesso limitato a quella BU
                        </small>
                    </div>
                    <div class="form-group">
...
function editUser(userId) {
    ...
    $.get('user_edit_ajax.php', {id: userId}, function(response) {
...
$('#searchUsers').on('keyup', function() {
    ...
    $('#usersTable tbody tr').each(function() {
        var rowText = '';
        // Cerca in email (colonna 2), BU (colonna 3), ragione sociale (colonna 4)
        rowText += $(this).find('td:nth-child(2)').text().toLowerCase() + ' ';
        rowText += $(this).find('td:nth-child(3)').text().toLowerCase() + ' ';
        rowText += $(this).find('td:nth-child(4)').text().toLowerCase();
    ...
    var visibleRows = $('#usersTable tbody tr:visible').length;
    if (visibleRows === 0 && searchText !== '') {
        if ($('#noResults').length === 0) {
            $('#usersTable tbody').append('<tr id="noResults"><td colspan="9" class="text-center text-muted"><i class="fas fa-search"></i> Nessun utente trovato</td></tr>');
        }
    } else {
        $('#noResults').remove();
    }
